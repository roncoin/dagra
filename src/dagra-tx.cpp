...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Dagra Core dagra-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  dagra-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded dagra transaction") + "\n" +
              "  dagra-tx [options] -create [commands]   " + _("Create hex-encoded dagra transaction") + "\n" +
              "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dagra transaction
